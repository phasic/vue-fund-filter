...
export default {
  ...
  computed: {
    ...
    filteredData: function() {
      return this._filterData(this.selectedMin, this.selectedMax, this.selectedSectors, this.selectedCurrencies);
    }
  },
  methods: {
    _getMockData() {
    ...
    _filterData(min, max, sectors, currencies) {
      const valuationFiltered = this.data.filter(el => el.valuation >= min && el.valuation <= max);
      const sectorFiltered = sectors.length === 0 ? valuationFiltered : valuationFiltered.filter(el => sectors && sectors.indexOf(el.sector) !== -1);
      const currencyFiltered = currencies.length === 0 ? sectorFiltered : sectorFiltered.filter(el => currencies && currencies.indexOf(el.currency) !== -1);
